...
const ODOO = {
  ...
  async login(login, password) {
    const r = await fetch(`${this.url}/web/session/authenticate`, {
      method:"POST", headers:{"Content-Type":"application/json"}, credentials:"include",
      body: JSON.stringify({ jsonrpc:"2.0", method:"call", id:1,
        params:{ db:this.db, login, password } }),
    });
    const d = await r.json();
    if (!d.result?.uid) throw new Error("Identifiants incorrects");
    return d.result;
  },
  async call(model, method, args) {
    const r = await fetch(`${this.url}/web/dataset/call_kw`, {
      method:"POST", headers:{"Content-Type":"application/json"}, credentials:"include",
      body: JSON.stringify({ jsonrpc:"2.0", method:"call", id:1,
  ...
  createOrder(partnerId, lines, note) {
    return this.call("sale.order","create",[{
      partner_id: partnerId, note,
      order_line: lines.map(l=>[0,0,{ product_id:l.id, product_uom_qty:l.qty, price_unit:l.price }]),
    }]);
  },
};
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode==="login") {
        // Try real Odoo first
        try {
          const result = await ODOO.login(form.email, form.password);
          onLogin({ name: result.name || form.email.split("@")[0], login: form.email, uid: result.uid });
        } catch {
          // Demo accounts
          const demos = {
            "demo@demo.com":"demo","m.alaoui@maisonnoir.ma":"Commercial@2024!",
            "admin@maisonnoir.ma":"MaisonNoir@2024!",
          };
          if (demos[form.email]===form.password || (form.email==="demo@demo.com"&&form.password==="demo")) {
            onLogin({ name:"Mehdi Alaoui", login:form.email, uid:1, mock:true });
          } else {
            throw new Error("Identifiants incorrects. Démo : demo@demo.com / demo");
          }
        }
      } else {
        ...
        // In real app: create Odoo partner + user
        onLogin({ name:form.name, login:form.email, uid:99, mock:true });
      }
...
export default function App() {
  ...
  const handleCheckout = async () => {
    ...
    const total = cart.reduce((s,i)=>s+i.price*i.qty,0);
    try {
      const id = await ODOO.createOrder(
        user.uid, cart.map(i=>({id:i.id, qty:i.qty, price:i.price})), "Commande boutique en ligne"
      );
      setSuccess({ orderId:`DEV/2024/${String(id).padStart(4,"0")}`, total });
    } catch {
      setSuccess({ orderId:null, total });
    }
    setCart([]);
    setCartOpen(false);
  };
